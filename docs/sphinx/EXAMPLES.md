...
You can pass parameters like `limit`, `sort`, and `offset` directly to the `query` and `query_category` methods as keyword arguments. These parameters will be appended to the query string in the format `|key=value`.
